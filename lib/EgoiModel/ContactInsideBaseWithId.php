...
/**
 * ContactInsideBaseWithId Class Doc Comment
 *
 * @category Class
 * @package  EgoiClient
 * @author   OpenAPI Generator team
 ...
 */
class ContactInsideBaseWithId implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'ContactInsideBaseWithId';
    ...
    */
    ...
    public function setContactId($contact_id)
    {
        ...
        if (!is_null($contact_id) && (!preg_match("/[a-fA-F\\d]{10}/", $contact_id))) {
            throw new \InvalidArgumentException("invalid value for \$contact_id when calling ContactInsideBaseWithId., must conform to the pattern /[a-fA-F\\d]{10}/.");
        }
    ...
    public function setSubscriptionForm($subscription_form)
    {
        ...
        if (!is_null($subscription_form) && ($subscription_form < 0)) {
            throw new \InvalidArgumentException('invalid value for $subscription_form when calling ContactInsideBaseWithId., must be bigger than or equal to 0.');
        }
